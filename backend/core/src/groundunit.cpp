...
/* Ground unit */
GroundUnit::GroundUnit(json json, unsigned int ID) : Unit(json, ID)
{
...
void GroundUnit::AIloop()
{
	...
	case State::SIMULATE_FIRE_FIGHT:
	{
		...
		if (internalCounter == 0 && targetPosition != Coords(NULL) && scheduler->getLoad() < 30)
		{
			...
			if (json_has_object_field(database, name))
			{
				json databaseEntry = database[name];
				if (json_has_number_field(databaseEntry, "aimTime"))
					aimTime = databaseEntry["aimTime"].template get<double>();
				if (json_has_boolean_field(databaseEntry, "indirectFire"))
					indirectFire = databaseEntry["indirectFire"].template get<bool>();
				if (json_has_number_field(databaseEntry, "shotsBaseInterval"))
					shotsBaseInterval = databaseEntry["shotsBaseInterval"].template get<double>();
			}
	...
	case State::MISS_ON_PURPOSE:
	{
		...
		{
			json databaseEntry = database[name];
			if (json_has_boolean_field(databaseEntry, "canAAA"))
				canAAA = databaseEntry["canAAA"].template get<bool>();
		}
		...
		if (canAAA)
		{
			...
			if (scheduler->getLoad() < 30 && internalCounter == 0)
			{
				...
				{
					json databaseEntry = database[name];
					if (json_has_number_field(databaseEntry, "barrelHeight"))
						barrelHeight = databaseEntry["barrelHeight"].template get<double>();
					if (json_has_number_field(databaseEntry, "muzzleVelocity"))
						muzzleVelocity = databaseEntry["muzzleVelocity"].template get<double>();
					if (json_has_number_field(databaseEntry, "aimTime"))
						aimTime = databaseEntry["aimTime"].template get<double>();
					if (json_has_number_field(databaseEntry, "shotsToFire"))
						shotsToFire = databaseEntry["shotsToFire"].template get<uint32_t>();
					if (json_has_number_field(databaseEntry, "engagementRange"))
						engagementRange = databaseEntry["engagementRange"].template get<double>();
					if (json_has_number_field(databaseEntry, "shotsBaseInterval"))
						shotsBaseInterval = databaseEntry["shotsBaseInterval"].template get<double>();
					if (json_has_number_field(databaseEntry, "shotsBaseScatter"))
						shotsBaseScatter = databaseEntry["shotsBaseScatter"].template get<double>();
					if (json_has_number_field(databaseEntry, "targetingRange"))
						targetingRange = databaseEntry["targetingRange"].template get<double>();
					if (json_has_number_field(databaseEntry, "aimMethodRange"))
						aimMethodRange = databaseEntry["aimMethodRange"].template get<double>();
					if (json_has_number_field(databaseEntry, "acquisitionRange"))
						acquisitionRange = databaseEntry["acquisitionRange"].template get<double>();
				}
			...
			if (internalCounter == 0)
			{
				...
				{
					json databaseEntry = database[name];
					if (json_has_number_field(databaseEntry, "alertnessTimeConstant"))
						alertnessTimeConstant = databaseEntry["alertnessTimeConstant"].template get<double>();
				}
...
void GroundUnit::aimAtPoint(Coords aimTarget)
{
	...
	{
		json databaseEntry = database[name];
		if (json_has_number_field(databaseEntry, "barrelHeight") && json_has_number_field(databaseEntry, "muzzleVelocity"))
		{
			barrelHeight = databaseEntry["barrelHeight"].template get<double>();
			muzzleVelocity = databaseEntry["muzzleVelocity"].template get<double>();
		}
